...
const AdminDiagnosisStatistics = () => {
  ...
  const exportToPDF = async () => {
    ...
    const fileName = `Статистика_захворювальності_за_${statistics.start_date}_${statistics.end_date}.pdf`;
    pdf.save(fileName);
  };

  return (
    <div className="doctor-visits-statistics-page">
      <h2>Статистика захворювальності</h2>
      ...
      {!loading && statistics && statistics.results.length > 0 && (
        <div ref={reportRef} className="pdf-report">
          <>
            ...
            <p>Загальна кількість випадків: {statistics.total_diagnoses}</p>
